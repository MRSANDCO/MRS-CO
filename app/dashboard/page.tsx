    ...
    const handleUpdateDriveLink = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/backend/admin/drive-links/${editingDriveLink.id}`, {
                ...
                body: JSON.stringify({
                    userId: editDriveUserId,
                    year: editDriveYear,
                    driveUrl: editDriveUrl,
                    title: editDriveTitle,
                    description: editDriveDescription,
                }),
            });
            if (!res.ok) throw new Error('Failed to update link');
            setDriveSuccess(`Drive link updated successfully!`);
            setEditingDriveLink(null);
            fetchDriveLinks();
        } catch (err: unknown) {
            setDriveError(err instanceof Error ? err.message : 'Failed to update link');
        } finally {
            setEditDriveSaving(false);
        }
    };
                    ...
                    <div className="space-y-6">
                        ...
                        {createSuccess && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                                ...
                                {createSuccess}
                                <button onClick={() => setCreateSuccess('')} className="ml-auto text-emerald-400/60 hover:text-emerald-400"><XCircle className="w-4 h-4" /></button>
                            </motion.div>
                        )}
                        {passwordChangeSuccess && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                                ...
                                <CheckCircle className="w-4 h-4 flex-shrink-0" />
                                {passwordChangeSuccess}
                                <button onClick={() => setPasswordChangeSuccess('')} className="ml-auto text-emerald-400/60 hover:text-emerald-400"><XCircle className="w-4 h-4" /></button>
                            </motion.div>
                        )}
                        {passwordChangeError && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                                ...
                                {passwordChangeError}
                                <button onClick={() => setPasswordChangeError('')} className="ml-auto text-red-400/60 hover:text-red-400"><XCircle className="w-4 h-4" /></button>
                            </motion.div>
                        )}
                        {createError && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                                ...
                                <AlertCircle className="w-4 h-4 flex-shrink-0" />
                                {createError}
                                <button onClick={() => setCreateError('')} className="ml-auto text-red-400/60 hover:text-red-400"><XCircle className="w-4 h-4" /></button>
                            </motion.div>
                        )}
                                ...
                                <CardContent className="p-6">
                                    <div className="flex items-center justify-between mb-5">
                                        ...
                                        <Button variant="ghost"
                                            ...
                                            className={`text-sm px-3 py-1.5 transition-all ${showCreateForm ? 'text-slate-400 hover:text-white hover:bg-white/[0.06]' : 'text-cyan-400 hover:text-cyan-300 hover:bg-cyan-500/10'}`}>
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                <div className="space-y-1.5">
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <Input required placeholder="e.g. john_doe" value={newUser.userId}
                                                ...
                                                <div className="space-y-1.5">
                                                    ...
                                                    <Input placeholder="e.g. John Doe" value={newUser.fullName}
                                                ...
                                                <div className="space-y-1.5 sm:col-span-2">
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <Input placeholder="e.g. +91 98765 43210" value={newUser.phone}
                                ...
                                <CardContent className="p-6">
                                    <div className="flex items-center justify-between mb-5">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Button variant="ghost" onClick={fetchUsers} disabled={loadingUsers}
                                    ...
                                    {loadingUsers ? (
                                        ...
                                        <div className="text-center py-16">
                                            ...
                                            <p className="text-slate-500 text-sm mt-1">Click &ldquo;+ New User&rdquo; above to create the first user.</p>
                                        </div>
                                    ) : (
                                                ...
                                                <tbody>
                                                    {adminUsers.map((u, index) => (
                                                        <motion.tr key={u.id || u.userId}
                                                            ...
                                                            <td className="py-3 px-3 text-slate-300 hidden sm:table-cell">{u.email || <span className="text-slate-600">—</span>}</td>
                                                            <td className="py-3 px-3 text-slate-300 hidden md:table-cell">{u.phone || <span className="text-slate-600">—</span>}</td>
                                                            <td className="py-3 px-3">
                                                                {u.active ? (
                                                            ...
                                                            <td className="py-3 px-3 text-slate-400 text-xs hidden lg:table-cell">{u.createdAt ? formatDate(u.createdAt) : '—'}</td>
                                                                ...
                                                                {changingPasswordFor === u.userId ? (
                                                                    ...
                                                                    <div className="flex items-center justify-end gap-1">
                                                                        <Button variant="ghost"
                                                                            ...
                                                                            className="h-8 px-2 text-slate-400 hover:text-amber-400 hover:bg-amber-500/10 transition-all" title="Change password">
                                                                            <KeyRound className="w-4 h-4" />
                                                                        </Button>
                                                                        <Button variant="ghost"
                                                                            onClick={() => { setEditingUser(u); setEditFullName(u.fullName || ''); setEditEmail(u.email || ''); setEditPhone(u.phone || ''); }}
                                                                            className="h-8 px-2 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-all" title="Edit User">
                                                                            <Edit className="w-4 h-4" />
                                                                        </Button>
                                                                        <Button variant="ghost" onClick={() => setUserToDelete(u.userId)}
                                                                            className="h-8 px-2 text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-all" title="Delete User">
                            ...
                            <Card className="border-white/[0.08] bg-white/[0.04] backdrop-blur-xl">
                                <CardContent className="p-6">
                                    <h2 className="text-lg font-semibold text-white flex items-center gap-2 mb-5">
                                        <UploadCloud className="w-5 h-5 text-violet-400" />Upload Documents
                                    </h2>
                                    {uploadSuccess && (
                                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                                            className="flex items-center gap-2 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm mb-4">
                                    ...
                                    <form onSubmit={handleUploadDocuments} className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                            <div className="space-y-1.5">
                                                ...
                                                <Input placeholder="Auto-filled from filename" value={uploadTitle} onChange={(e) => setUploadTitle(e.target.value)}
                                                    className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-slate-600 focus:border-cyan-500/50 focus:ring-cyan-500/20" />
                                            </div>
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Category</label>
                                                <Input placeholder="e.g. Tax, Legal" value={uploadCategory} onChange={(e) => setUploadCategory(e.target.value)}
                                                    className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-slate-600 focus:border-cyan-500/50 focus:ring-cyan-500/20" />
                                            </div>
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Description</label>
                                                <Input placeholder="Optional description" value={uploadDescription} onChange={(e) => setUploadDescription(e.target.value)}
                                                    className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-slate-600 focus:border-cyan-500/50 focus:ring-cyan-500/20" />
                                            </div>
                                        </div>
                                        <div className="flex justify-end pt-2">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.35 }}>
                            <Card className="border-white/[0.08] bg-white/[0.04] backdrop-blur-xl">
                                <CardContent className="p-6">
                                    <h2 className="text-lg font-semibold text-white flex items-center gap-2 mb-5">
                                        <FolderOpen className="w-5 h-5 text-blue-400" />Share Google Drive Folder
                                    </h2>
                                    {driveSuccess && (
                                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                                            ...
                                            <CheckCircle className="w-4 h-4 flex-shrink-0" />{driveSuccess}
                                            <button onClick={() => setDriveSuccess('')} className="ml-auto text-emerald-400/60 hover:text-emerald-400"><XCircle className="w-4 h-4" /></button>
                                        </motion.div>
                                    )}
                                    {driveError && (
                                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                                            className="flex items-center gap-2 p-3 mb-4 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm">
                                            <AlertCircle className="w-4 h-4 flex-shrink-0" />{driveError}
                                            <button onClick={() => setDriveError('')} className="ml-auto text-red-400/60 hover:text-red-400"><XCircle className="w-4 h-4" /></button>
                                        </motion.div>
                                    )}
                                    <form onSubmit={handleSaveDriveLink} className="space-y-4">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Assign to Client *</label>
                                            ...
                                            <div className="space-y-1.5">
                                                ...
                                                <Input required placeholder="e.g. 2024" value={driveYear} onChange={(e) => setDriveYear(e.target.value)}
                                                    className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-slate-600 focus:border-cyan-500/50 focus:ring-cyan-500/20" />
                                            </div>
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Google Drive Link *</label>
                                            <Input required placeholder="https://drive.google.com/drive/folders/..." value={driveUrl} onChange={(e) => setDriveUrl(e.target.value)}
                                                className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-slate-600 focus:border-cyan-500/50 focus:ring-cyan-500/20" />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Folder Name</label>
                                            <Input placeholder="e.g. Tax Documents 2024" value={driveTitle} onChange={(e) => setDriveTitle(e.target.value)}
                                                className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-slate-600 focus:border-cyan-500/50 focus:ring-cyan-500/20" />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Description</label>
                                            <textarea rows={3} placeholder="Optional description for the client..."
                                        ...
                                        <div className="flex justify-end pt-2">
                                            <Button type="submit" disabled={driveSaving}
                                                ...
                                                {driveSaving ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Saving...</> : <><FolderOpen className="w-4 h-4 mr-2" />Share Folder Link</>}
                                            </Button>
                                        </div>
                                    </form>
                                </CardContent>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>
                            <Card className="border-white/[0.08] bg-white/[0.04] backdrop-blur-xl">
                                <CardContent className="p-6">
                                    <div className="flex items-center justify-between mb-5">
                                        <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                                            <FileText className="w-5 h-5 text-orange-400" />Documents
                                        </h2>
                                        <div className="flex items-center gap-2">
                                            <select value={docsFilterUser} onChange={(e) => setDocsFilterUser(e.target.value)}
                                                ...
                                                {adminUsers.map(u => (
                                                    <option key={u.userId} value={u.userId} className="bg-slate-900">{u.fullName || u.userId}</option>
                                                ))}
                                            </select>
                                            <span className="text-xs text-slate-500 bg-white/[0.04] px-2.5 py-1 rounded-full">
                                                            ...
                                                            <td className="py-3 px-3 text-right">
                                                                <div className="flex items-center justify-end gap-1">
                                                                    <Button variant="ghost" onClick={() => handleDownloadDoc(doc)} disabled={downloadingDocId === doc.id}
                                                                        className="h-8 px-2 text-slate-400 hover:text-cyan-400 hover:bg-cyan-500/10 transition-all" title="Download Document">
                                                                        {downloadingDocId === doc.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                                                                    </Button>
                                                                    <Button variant="ghost" onClick={() => setDocToDelete(doc.id)}
                                                                        className="h-8 px-2 text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-all" title="Delete Document">
                                                                        <Trash2 className="w-4 h-4" />
                                                                    </Button>
                                                                </div>
                                                            </td>
                        ...
                        {/* ── Shared Drive Links List ── */}
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.45 }}>
                            <Card className="border-white/[0.08] bg-white/[0.04] backdrop-blur-xl">
                                <CardContent className="p-6">
                                    <div className="flex items-center justify-between mb-5">
                                        <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                                            <FolderOpen className="w-5 h-5 text-blue-400" />Shared Drive Links
                                        </h2>
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-slate-500 bg-white/[0.04] px-2.5 py-1 rounded-full">
                                                {driveLinks.length} {driveLinks.length === 1 ? 'link' : 'links'}
                                            </span>
                                            <Button variant="ghost" onClick={fetchDriveLinks} disabled={loadingDriveLinks}
                                                className="text-slate-400 hover:text-white hover:bg-white/[0.06] px-2">
                                    ...
                                    {loadingDriveLinks ? (
                                        ...
                                        <div className="text-center py-16">
                                            ...
                                            <p className="text-slate-400 font-medium">No drive links shared yet</p>
                                            <p className="text-slate-500 text-sm mt-1">Use the form above to share a Google Drive folder with a client.</p>
                                        </div>
                                    ) : (
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-sm">
                                                <thead>
                                                    <tr className="border-b border-white/[0.06]">
                                                        <th className="text-left py-3 px-3 text-slate-400 font-medium text-xs uppercase tracking-wider">Folder</th>
                                                        <th className="text-left py-3 px-3 text-slate-400 font-medium text-xs uppercase tracking-wider hidden sm:table-cell">Client</th>
                                                        <th className="text-left py-3 px-3 text-slate-400 font-medium text-xs uppercase tracking-wider hidden md:table-cell">Year</th>
                                                        <th className="text-left py-3 px-3 text-slate-400 font-medium text-xs uppercase tracking-wider hidden lg:table-cell">Description</th>
                                                        <th className="text-right py-3 px-3 text-slate-400 font-medium text-xs uppercase tracking-wider">Actions</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {driveLinks.map((link, index) => (
                                                        <motion.tr key={link.id || index}
                                                            initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
                                                            transition={{ duration: 0.2, delay: index * 0.03 }}
                                                            className="border-b border-white/[0.03] hover:bg-white/[0.03] transition-colors">
                                                            <td className="py-3 px-3">
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <a href={link.driveUrl} target="_blank" rel="noopener noreferrer"
                                                            ...
                                                            <td className="py-3 px-3 text-slate-300 hidden sm:table-cell">
                                                                <span className="inline-flex items-center gap-1.5">
                                                                    <User className="w-3 h-3 text-slate-500" />{link.userId}
                                                                </span>
                                                            </td>
                                                            ...
                                                            <td className="py-3 px-3 text-slate-400 text-xs hidden lg:table-cell max-w-[200px]">
                                                                <span className="truncate block">{link.description || '—'}</span>
                                                            </td>
                                                            <td className="py-3 px-3 text-right">
                                                                <div className="flex items-center justify-end gap-1">
                                                                    <Button variant="ghost" onClick={() => window.open(link.driveUrl, '_blank')}
                                                                        className="h-8 px-2 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-all" title="Open Drive Folder">
                                                                        <Download className="w-4 h-4" />
                                                                    </Button>
                                                                    <Button variant="ghost"
                                                                        onClick={() => {
                                                                            setEditingDriveLink(link);
                                                                            setEditDriveUserId(link.userId || '');
                                                                            setEditDriveYear(link.year || '');
                                                                            setEditDriveUrl(link.driveUrl || '');
                                                                            setEditDriveTitle(link.title || '');
                                                                            setEditDriveDescription(link.description || '');
                                                                        }}
                                                                        className="h-8 px-2 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-all" title="Edit Link">
                                                                        <Edit className="w-4 h-4" />
                                                                    </Button>
                                                                    <Button variant="ghost" onClick={() => handleDeleteDriveLink(link.id)}
                                                                        className="h-8 px-2 text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-all" title="Delete Link">
                                                                        <Trash2 className="w-4 h-4" />
                                                                    </Button>
                                                                </div>
                                                            </td>
                            ...
                            <Card className="border-white/[0.08] bg-white/[0.04] backdrop-blur-xl h-full">
                                <CardContent className="p-6">
                                    ...
                                    {loadingProfile ? (
                                        ...
                                        <div className="flex items-center gap-2 p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm">
                                            <XCircle className="w-4 h-4 flex-shrink-0" />{profileError}
                                        </div>
                                    ) : profile ? (
                                        <div className="space-y-4">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex items-center gap-3 text-sm">
                                                    ...
                                                    <span className="text-slate-300">Joined {profile.createdAt ? formatDate(profile.createdAt) : 'N/A'}</span>
                                                </div>
                                                <div className="flex items-center gap-3 text-sm">
                                                    {profile.active ? (
                                                        <><CheckCircle className="w-4 h-4 text-emerald-400 flex-shrink-0" /><span className="text-emerald-400 font-medium">Active</span></>
                                                    ) : (
                                                        <><XCircle className="w-4 h-4 text-red-400 flex-shrink-0" /><span className="text-red-400 font-medium">Inactive</span></>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <Card className="border-white/[0.08] bg-white/[0.04] backdrop-blur-xl">
                                <CardContent className="p-6">
                                    ...
                                    {loadingDocs ? (
                                        ...
                                        <div className="flex items-center gap-2 p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm">
                                            <XCircle className="w-4 h-4 flex-shrink-0" />{docsError}
                                        </div>
                                    ) : documents.length === 0 ? (
                                        <div className="text-center py-16">
                                            <FolderOpen className="w-12 h-12 text-slate-600 mx-auto mb-3" />
                                            <p className="text-slate-400 font-medium">No documents yet</p>
                                            <p className="text-slate-500 text-sm mt-1">Documents uploaded by admin will appear here.</p>
                                        </div>
                                    ) : (
                                        <div className="space-y-3">
                                ...
                                <CardContent className="p-6">
                                    ...
                                    {loadingClientDriveLinks ? (
                                        ...
                                        <div className="text-center py-16">
                                            ...
                                            <p className="text-slate-500 text-sm mt-1">Folders shared by admin will appear here.</p>
                                        </div>
                                    ) : (
                                        <div className="space-y-3">
                                            {clientDriveLinks.map((link, index) => (
                                                <motion.div key={link.id || index}
                                                    initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                                                    transition={{ duration: 0.3, delay: index * 0.05 }}
                                                    className="group flex items-center gap-4 p-4 rounded-xl bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.06] hover:border-white/[0.1] transition-all duration-300">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-1.5">
                                ...
                                <Input value={editFullName} onChange={e => setEditFullName(e.target.value)}
                                    className="bg-white/[0.04] text-white border-white/10 focus:border-cyan-500/50" placeholder="e.g. John Doe" />
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-xs text-slate-400 uppercase tracking-wider">Email</label>
                                <Input value={editEmail} onChange={e => setEditEmail(e.target.value)} type="email"
                                    className="bg-white/[0.04] text-white border-white/10 focus:border-cyan-500/50" placeholder="john@example.com" />
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-xs text-slate-400 uppercase tracking-wider">Phone</label>
                                <Input value={editPhone} onChange={e => setEditPhone(e.target.value)}
                                    className="bg-white/[0.04] text-white border-white/10 focus:border-cyan-500/50" placeholder="+91 98765 43210" />
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
                    <div className="bg-slate-900 border border-red-500/20 rounded-2xl p-6 w-full max-w-sm">
                        ...
                        <div className="flex gap-3 justify-end">
                            ...
                            <Button variant="destructive" onClick={handleDeleteDoc} disabled={deletingDoc}
                                className="bg-red-500 hover:bg-red-600 text-white shadow-lg shadow-red-500/20">
            ...
            {editingDriveLink && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
                    <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-md">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                <FolderOpen className="w-5 h-5 text-blue-400" /> Edit Drive Link
                            </h3>
                            <Button variant="ghost" onClick={() => setEditingDriveLink(null)}
                                className="h-8 w-8 p-0 text-slate-400 hover:text-white hover:bg-white/10 rounded-full">
                                <XCircle className="w-5 h-5" />
                            </Button>
                        </div>
                        <form onSubmit={handleUpdateDriveLink} className="space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <label className="text-xs text-slate-400 uppercase tracking-wider">Assign to Client *</label>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Input required placeholder="e.g. 2024" value={editDriveYear} onChange={(e) => setEditDriveYear(e.target.value)}
                                        className="bg-white/[0.04] text-white border-white/10 focus:border-cyan-500/50" />
                                </div>
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-xs text-slate-400 uppercase tracking-wider">Google Drive Link *</label>
                                <Input required placeholder="https://drive.google.com/drive/folders/..." value={editDriveUrl} onChange={(e) => setEditDriveUrl(e.target.value)}
                                    className="bg-white/[0.04] text-white border-white/10 focus:border-cyan-500/50" />
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-xs text-slate-400 uppercase tracking-wider">Folder Name</label>
                                <Input placeholder="e.g. Tax Documents 2024" value={editDriveTitle} onChange={(e) => setEditDriveTitle(e.target.value)}
                                    className="bg-white/[0.04] text-white border-white/10 focus:border-cyan-500/50" />
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-xs text-slate-400 uppercase tracking-wider">Description</label>
                                <textarea rows={3} placeholder="Optional description..."
                                    value={editDriveDescription} onChange={(e) => setEditDriveDescription(e.target.value)}
                                    className="w-full px-3 py-2 rounded-md bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-slate-600 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 outline-none text-sm resize-none" />
                            </div>
                            <div className="flex gap-3 justify-end pt-2">
                                ...
                                <Button type="submit" disabled={editDriveSaving}
                                    className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white shadow-lg shadow-cyan-500/20">
