...
import { loginUser, loginAdmin, loginEmployee } from '@/lib/api';
import {
    Shield,
    User,
    Briefcase,
...
type TabType = 'user' | 'employee' | 'admin';
...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (activeTab === 'employee') {
                const res = await loginEmployee(userId.trim(), password);
                login({
                    userId: res.employeeId || userId.trim(),
                    fullName: res.name || res.employeeId || 'Employee',
                    role: 'employee',
                    token: res.token,
                ...
                const res = await loginUser(userId.trim(), password);
                login({
                    userId: res.userId || userId.trim(),
                ...
                const res = await loginAdmin(userId.trim(), password);
                login({
                    userId: userId.trim(),
    ...
    const tabConfig: { id: TabType; label: string; icon: React.ReactNode; placeholder: string; inputLabel: string }[] = [
        {
            id: 'user',
            label: 'Client',
            icon: <User className="w-3.5 h-3.5" />,
            placeholder: 'Enter your Client / User ID',
            inputLabel: 'User ID',
        },
        {
            id: 'employee',
    ...
    const currentTab = tabConfig.find((t) => t.id === activeTab) || tabConfig[0];
    ...
    return (
        <div className="min-h-screen relative flex items-center justify-center px-4 py-8 overflow-hidden">
            ...
            >
                ...
                <Link
                    href="/"
                    className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-200 transition-colors mb-6 text-sm tracking-wide"
                >
                ...
                <div className="text-center mb-6">
                    ...
                    >
                        ...
                        <div className="relative mb-3">
                            ...
                            />
                            <div
                                className="relative w-18 h-18 sm:w-20 sm:h-20 rounded-full flex items-center justify-center"
                                style={{
                    ...
                    >
                        <h1 className="text-xl sm:text-2xl font-semibold text-white mb-1" style={{ letterSpacing: '-0.01em' }}>
                            Welcome Back
                        </h1>
                ...
                >
                    ...
                    >
                        ...
                        <div
                            ...
                            style={{
                                background: 'linear-gradient(90deg, transparent 5%, rgba(255,255,255,0.15) 40%, rgba(99,210,255,0.2) 60%, transparent 95%)',
                            }}
                        />

                        <CardContent className="p-6 sm:p-7">
                            {/* Tabs - 3 Options: Client | Employee | Admin */}
                            <div
                                className="grid grid-cols-3 rounded-xl p-1 mb-6"
                                style={{
                                ...
                                {tabConfig.map((tab) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className="relative flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs sm:text-sm font-medium transition-all duration-300"
                                            style={{
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <button
                                            ...
                                            className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white transition-colors duration-150"
                                ...
                                {error && (
                                    <motion.div
                                        ...
                                        className="flex items-center gap-2.5 p-3 rounded-xl text-xs sm:text-sm"
                                        style={{
                                ...
                                <Button
                                    type="submit"
                                    disabled={loading}
                                    className="w-full h-11 text-white font-semibold rounded-xl text-sm tracking-wide transition-all duration-300 hover:scale-[1.015] active:scale-[0.985] disabled:opacity-60 disabled:scale-100 mt-2"
                                    style={{
