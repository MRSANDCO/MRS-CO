...
import { Award, Shield, CheckCircle, ChevronLeft, ChevronRight } from 'lucide-react';
import Image from 'next/image';
...
const AchievementSection = memo(() => {
  ...
  // Certificate data - single source of truth for the horizontal scroller.
  const certificates = [
    {
      src: "/assets/certificates/peer-review-certificate-v2.png",
      alt: "ICAI Peer Review Certificate",
    ...
    {
      src: "/assets/certificates/Uaecorporatetax.png",
      alt: "UAE Corporate Tax Certificate",
      title: "UAE Corporate Tax Certificate",
      issuer: "The Institute of Chartered Accountants of India (ICAI)",
      badges: [
        { text: "ICAI Certified", variant: "indigo" },
  ...
  return (
    <section className="relative py-20 overflow-hidden w-full">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        {/* Verified Accreditations heading */}
        <motion.div
          initial={fadeInSide.initial}
          whileInView={fadeInSide.whileInView}
          transition={fadeInSide.transition}
        ...
        >
          ...
          <p className="text-gray-400 text-sm">Scroll horizontally or use the arrows to view each certificate</p>
        </motion.div>

        {/* Horizontal Scroll Certificate Carousel */}
...
// ---------------------------------------------------------------------------
// Carousel: horizontal full-screen viewport tracking elements
// ---------------------------------------------------------------------------
interface CertData {
  src: string;
  alt: string;
...
const CertificateCarousel = memo<CertificateCarouselProps>(({ certificates }) => {
  const [activeIndex, setActiveIndex] = useState(0);
  const containerRef = useRef<HTMLDivElement>(null);

  // Calculate active index on scroll cleanly
  const handleScroll = useCallback(() => {
    ...
    if (itemWidth > 0) {
      ...
      if (index >= 0 && index < certificates.length && index !== activeIndex) {
  ...
  const goTo = useCallback((nextIndex: number) => {
    const el = containerRef.current;
    if (!el) return;
    const targetScroll = nextIndex * el.clientWidth;
    el.scrollTo({ left: targetScroll, behavior: 'smooth' });
    setActiveIndex(nextIndex);
  }, []);

  const goNext = useCallback(() => {
    goTo(Math.min(activeIndex + 1, certificates.length - 1));
  }, [activeIndex, goTo, certificates.length]);
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto">
      ...
      <button
        ...
        className="hidden md:flex absolute -left-16 lg:-left-20 top-1/2 -translate-y-1/2 z-20 items-center justify-center w-12 h-12 rounded-full bg-white/10 backdrop-blur-lg border border-white/20 text-white hover:bg-white/20 hover:border-cyan-400/50 transition-all duration-300 disabled:opacity-30 disabled:cursor-not-allowed"
      >
      ...
      {/* Isolated display track - width forced to 100% to ensure only one element handles view area */}
      <div
        ref={containerRef}
        ...
        className="relative flex overflow-x-auto snap-x snap-mandatory scroll-smooth py-6 px-0 focus:outline-none no-scrollbar w-full"
        style={{ WebkitOverflowScrolling: 'touch' }}
      >
        {certificates.map((cert) => (
          <div
            key={cert.title}
            className="w-full max-w-full flex-shrink-0 snap-center flex justify-center items-center px-4"
          >
            <CertificateCard {...cert} delay={0} />
          </div>
        ))}
      </div>
        ...
        {certificates.map((cert, idx) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-300 ${
              idx === activeIndex ? 'w-8 bg-cyan-400' : 'w-2 bg-white/30 hover:bg-white/50'
            }`}
          />
        ))}
      </div>
...
// ---------------------------------------------------------------------------
// Image Only Display — Size adjusts cleanly and strictly per image dimensions
// ---------------------------------------------------------------------------
interface CertificateCardProps {
  src: string;
  alt: string;
...
const CertificateCard = memo<CertificateCardProps>(({
  ...
  return (
    ...
    >
      {/* Container sizing dynamically matching image aspects without extra frames */}
      <div className={`w-full relative mx-auto ${isPortrait ? 'max-w-md' : 'max-w-2xl'}`}>
        <div style={{ aspectRatio: `${width} / ${height}` }} className="w-full relative shadow-2xl rounded-lg overflow-hidden">
          <Image
            src={src}
            alt={alt}
            fill
      ...
      <div className="text-center mt-6 w-full max-w-xl px-4">
        <div className="flex items-center justify-center mb-2">
          <h4 className="font-bold text-xl md:text-2xl text-white">
            {title}
          </h4>
          {Icon ? (
            <Icon className={`w-5 h-5 ${iconColor} ml-2 shrink-0`} />
          ) : (
            <CheckCircle className={`w-5 h-5 ${iconColor} ml-2 shrink-0`} />
          )}
        </div>
        ...
        <div className="flex items-center justify-center gap-3 flex-wrap">
          {badges.map((badge) => (
            <span
              key={badge.text}
              className={`px-4 py-1 rounded-full text-xs font-semibold border ${badgeColors[badge.variant]}`}
            >
              {badge.text}
            </span>
          ))}
